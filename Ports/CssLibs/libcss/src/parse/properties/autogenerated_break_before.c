		...
		css_style *result)
{
	...
	if ((lwc_string_caseless_isequal(
		...
		error = css__stylesheet_style_appendOPV(result,
				CSS_PROP_BREAK_BEFORE,
				0,BREAK_BEFORE_ALWAYS);

	} else if ((lwc_string_caseless_isequal(
			token->idata, c->strings[AVOID],
			&match) == lwc_error_ok && match)) {
		error = css__stylesheet_style_appendOPV(result,
				CSS_PROP_BREAK_BEFORE,
				0,BREAK_BEFORE_AVOID);
		...
		error = css__stylesheet_style_appendOPV(result,
				CSS_PROP_BREAK_BEFORE,
				0,BREAK_BEFORE_RIGHT);

	} else if ((lwc_string_caseless_isequal(
			token->idata, c->strings[PAGE],
			&match) == lwc_error_ok && match)) {
		error = css__stylesheet_style_appendOPV(result,
				CSS_PROP_BREAK_BEFORE,
				0,BREAK_BEFORE_PAGE);
		...
		error = css__stylesheet_style_appendOPV(result,
				CSS_PROP_BREAK_BEFORE,
				0,BREAK_BEFORE_COLUMN);

	} else if ((lwc_string_caseless_isequal(
			token->idata, c->strings[AVOID_PAGE],
			&match) == lwc_error_ok && match)) {
		error = css__stylesheet_style_appendOPV(result,
				CSS_PROP_BREAK_BEFORE,
				0,BREAK_BEFORE_AVOID_PAGE);

	} else if ((lwc_string_caseless_isequal(
			token->idata, c->strings[AVOID_COLUMN],
			&match) == lwc_error_ok && match)) {
		error = css__stylesheet_style_appendOPV(result,
				CSS_PROP_BREAK_BEFORE,
				0,BREAK_BEFORE_AVOID_COLUMN);
