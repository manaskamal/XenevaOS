; Listing generated by Microsoft (R) Optimizing Compiler Version 19.29.30154.0 

include listing.inc

INCLUDELIB LIBCMT
INCLUDELIB OLDNAMES

PUBLIC	?__ps2mouse@@3PEAU__ps2mouse__@@EA		; __ps2mouse
_BSS	SEGMENT
?__ps2mouse@@3PEAU__ps2mouse__@@EA DQ 01H DUP (?)	; __ps2mouse
_BSS	ENDS
msvcjmc	SEGMENT
__5B21646B_mouse@cpp DB 01H
msvcjmc	ENDS
PUBLIC	?AuPS2MouseInitialise@@YAXXZ			; AuPS2MouseInitialise
PUBLIC	?AuPS2MouseSetPos@@YAXHH@Z			; AuPS2MouseSetPos
PUBLIC	AuPS2DisableMouse
PUBLIC	?PS2MouseWaitInput@@YAXXZ			; PS2MouseWaitInput
PUBLIC	?PS2MouseWaitOutput@@YAXXZ			; PS2MouseWaitOutput
PUBLIC	?AuPS2MouseWrite@@YAEE@Z			; AuPS2MouseWrite
PUBLIC	?AuPS2MouseRead@@YAEXZ				; AuPS2MouseRead
PUBLIC	?PS2MouseHandler@@YAX_KPEAX@Z			; PS2MouseHandler
PUBLIC	__JustMyCode_Default
PUBLIC	??_C@_0BB@LLAPAKF@Right?5clicked?5?$AN?6@	; `string'
PUBLIC	??_C@_0BO@EPKMAGCI@PS2?5Mouse?5with?5scroll?5wheel?5?6@ ; `string'
EXTRN	AuHalRegisterIRQ:PROC
EXTRN	AuInterruptEnd:PROC
EXTRN	x64_cli:PROC
EXTRN	x64_sti:PROC
EXTRN	x64_inportb:PROC
EXTRN	x64_outportb:PROC
EXTRN	kmalloc:PROC
EXTRN	memset:PROC
EXTRN	memcpy:PROC
EXTRN	AuTextOut:PROC
EXTRN	AuDevWriteMice:PROC
EXTRN	SeTextOut:PROC
EXTRN	__CheckForDebuggerJustMyCode:PROC
;	COMDAT pdata
pdata	SEGMENT
$pdata$?AuPS2MouseInitialise@@YAXXZ DD imagerel $LN4
	DD	imagerel $LN4+345
	DD	imagerel $unwind$?AuPS2MouseInitialise@@YAXXZ
pdata	ENDS
;	COMDAT pdata
pdata	SEGMENT
$pdata$?AuPS2MouseSetPos@@YAXHH@Z DD imagerel $LN4
	DD	imagerel $LN4+74
	DD	imagerel $unwind$?AuPS2MouseSetPos@@YAXHH@Z
pdata	ENDS
;	COMDAT pdata
pdata	SEGMENT
$pdata$AuPS2DisableMouse DD imagerel $LN3
	DD	imagerel $LN3+51
	DD	imagerel $unwind$AuPS2DisableMouse
pdata	ENDS
;	COMDAT pdata
pdata	SEGMENT
$pdata$?PS2MouseWaitInput@@YAXXZ DD imagerel $LN6
	DD	imagerel $LN6+78
	DD	imagerel $unwind$?PS2MouseWaitInput@@YAXXZ
pdata	ENDS
;	COMDAT pdata
pdata	SEGMENT
$pdata$?PS2MouseWaitOutput@@YAXXZ DD imagerel $LN6
	DD	imagerel $LN6+73
	DD	imagerel $unwind$?PS2MouseWaitOutput@@YAXXZ
pdata	ENDS
;	COMDAT pdata
pdata	SEGMENT
$pdata$?AuPS2MouseWrite@@YAEE@Z DD imagerel $LN3
	DD	imagerel $LN3+80
	DD	imagerel $unwind$?AuPS2MouseWrite@@YAEE@Z
pdata	ENDS
;	COMDAT pdata
pdata	SEGMENT
$pdata$?AuPS2MouseRead@@YAEXZ DD imagerel $LN3
	DD	imagerel $LN3+43
	DD	imagerel $unwind$?AuPS2MouseRead@@YAEXZ
pdata	ENDS
;	COMDAT pdata
pdata	SEGMENT
$pdata$?PS2MouseHandler@@YAX_KPEAX@Z DD imagerel $LN31
	DD	imagerel $LN31+1241
	DD	imagerel $unwind$?PS2MouseHandler@@YAX_KPEAX@Z
pdata	ENDS
;	COMDAT ??_C@_0BO@EPKMAGCI@PS2?5Mouse?5with?5scroll?5wheel?5?6@
CONST	SEGMENT
??_C@_0BO@EPKMAGCI@PS2?5Mouse?5with?5scroll?5wheel?5?6@ DB 'PS2 Mouse wit'
	DB	'h scroll wheel ', 0aH, 00H			; `string'
CONST	ENDS
;	COMDAT ??_C@_0BB@LLAPAKF@Right?5clicked?5?$AN?6@
CONST	SEGMENT
??_C@_0BB@LLAPAKF@Right?5clicked?5?$AN?6@ DB 'Right clicked ', 0dH, 0aH, 00H ; `string'
CONST	ENDS
;	COMDAT xdata
xdata	SEGMENT
$unwind$?PS2MouseHandler@@YAX_KPEAX@Z DD 025041701H
	DD	01122317H
	DD	0500b0014H
xdata	ENDS
;	COMDAT xdata
xdata	SEGMENT
$unwind$?AuPS2MouseRead@@YAEXZ DD 025030b01H
	DD	0b206230bH
	DD	05002H
xdata	ENDS
;	COMDAT xdata
xdata	SEGMENT
$unwind$?AuPS2MouseWrite@@YAEE@Z DD 025030e01H
	DD	0b209230eH
	DD	05005H
xdata	ENDS
;	COMDAT xdata
xdata	SEGMENT
$unwind$?PS2MouseWaitOutput@@YAXXZ DD 025030b01H
	DD	0d206230bH
	DD	05002H
xdata	ENDS
;	COMDAT xdata
xdata	SEGMENT
$unwind$?PS2MouseWaitInput@@YAXXZ DD 025030b01H
	DD	0d206230bH
	DD	05002H
xdata	ENDS
;	COMDAT xdata
xdata	SEGMENT
$unwind$AuPS2DisableMouse DD 025030b01H
	DD	0b206230bH
	DD	05002H
xdata	ENDS
;	COMDAT xdata
xdata	SEGMENT
$unwind$?AuPS2MouseSetPos@@YAXHH@Z DD 025031201H
	DD	0b20d2312H
	DD	05009H
xdata	ENDS
;	COMDAT xdata
xdata	SEGMENT
$unwind$?AuPS2MouseInitialise@@YAXXZ DD 025030b01H
	DD	0d206230bH
	DD	05002H
xdata	ENDS
; Function compile flags: /Odt
;	COMDAT __JustMyCode_Default
_TEXT	SEGMENT
__JustMyCode_Default PROC				; COMDAT
	ret	0
__JustMyCode_Default ENDP
_TEXT	ENDS
; Function compile flags: /Odtp /ZI
; File E:\Xeneva Project\Aurora\Kernel\Drivers\mouse.cpp
;	COMDAT ?PS2MouseHandler@@YAX_KPEAX@Z
_TEXT	SEGMENT
status$ = 0
mouse_in$1 = 1
x$2 = 4
y$3 = 8
newmsg$4 = 16
tv76 = 112
v$ = 144
p$ = 152
?PS2MouseHandler@@YAX_KPEAX@Z PROC			; PS2MouseHandler, COMDAT

; 109  : void PS2MouseHandler(size_t v, void* p) {

$LN31:
	mov	QWORD PTR [rsp+16], rdx
	mov	QWORD PTR [rsp+8], rcx
	push	rbp
	sub	rsp, 160				; 000000a0H
	lea	rbp, QWORD PTR [rsp+32]
	lea	rcx, OFFSET FLAT:__5B21646B_mouse@cpp
	call	__CheckForDebuggerJustMyCode

; 110  : 	x64_cli();

	call	x64_cli

; 111  : 	uint8_t status = x64_inportb(MOUSE_STATUS);

	mov	cx, 100					; 00000064H
	call	x64_inportb
	mov	BYTE PTR status$[rbp], al
$LN2@PS2MouseHa:

; 112  : 	while ((status & MOUSE_BBIT) && (status & MOUSE_F_BIT)) {

	movzx	eax, BYTE PTR status$[rbp]
	and	eax, 1
	test	eax, eax
	je	$LN3@PS2MouseHa
	movzx	eax, BYTE PTR status$[rbp]
	and	eax, 32					; 00000020H
	test	eax, eax
	je	$LN3@PS2MouseHa

; 113  : 		int8_t mouse_in = x64_inportb(MOUSE_PORT);

	mov	cx, 96					; 00000060H
	call	x64_inportb
	mov	BYTE PTR mouse_in$1[rbp], al

; 114  : 		switch (__ps2mouse->mouse_cycle) {

	mov	rax, QWORD PTR ?__ps2mouse@@3PEAU__ps2mouse__@@EA ; __ps2mouse
	movzx	eax, BYTE PTR [rax]
	mov	BYTE PTR tv76[rbp], al
	cmp	BYTE PTR tv76[rbp], 0
	je	SHORT $LN6@PS2MouseHa
	cmp	BYTE PTR tv76[rbp], 1
	je	SHORT $LN8@PS2MouseHa
	cmp	BYTE PTR tv76[rbp], 2
	je	$LN9@PS2MouseHa
	cmp	BYTE PTR tv76[rbp], 3
	je	$LN12@PS2MouseHa
	cmp	BYTE PTR tv76[rbp], 4
	je	$LN14@PS2MouseHa
	jmp	$LN4@PS2MouseHa
$LN6@PS2MouseHa:

; 115  : 		case 0:
; 116  : 			__ps2mouse->mouse_byte[0] = mouse_in;

	mov	eax, 1
	imul	rax, rax, 0
	mov	rcx, QWORD PTR ?__ps2mouse@@3PEAU__ps2mouse__@@EA ; __ps2mouse
	movzx	edx, BYTE PTR mouse_in$1[rbp]
	mov	BYTE PTR [rcx+rax+1], dl

; 117  : 			if (!(mouse_in & MOUSE_V_BIT)) break;

	movsx	eax, BYTE PTR mouse_in$1[rbp]
	and	eax, 8
	test	eax, eax
	jne	SHORT $LN7@PS2MouseHa
	jmp	$LN4@PS2MouseHa
$LN7@PS2MouseHa:

; 118  : 			++__ps2mouse->mouse_cycle;

	mov	rax, QWORD PTR ?__ps2mouse@@3PEAU__ps2mouse__@@EA ; __ps2mouse
	movzx	eax, BYTE PTR [rax]
	inc	al
	mov	rcx, QWORD PTR ?__ps2mouse@@3PEAU__ps2mouse__@@EA ; __ps2mouse
	mov	BYTE PTR [rcx], al

; 119  : 			break;

	jmp	$LN4@PS2MouseHa
$LN8@PS2MouseHa:

; 120  : 		case 1:
; 121  : 			__ps2mouse->mouse_byte[1] = mouse_in;

	mov	eax, 1
	imul	rax, rax, 1
	mov	rcx, QWORD PTR ?__ps2mouse@@3PEAU__ps2mouse__@@EA ; __ps2mouse
	movzx	edx, BYTE PTR mouse_in$1[rbp]
	mov	BYTE PTR [rcx+rax+1], dl

; 122  : 			++__ps2mouse->mouse_cycle;

	mov	rax, QWORD PTR ?__ps2mouse@@3PEAU__ps2mouse__@@EA ; __ps2mouse
	movzx	eax, BYTE PTR [rax]
	inc	al
	mov	rcx, QWORD PTR ?__ps2mouse@@3PEAU__ps2mouse__@@EA ; __ps2mouse
	mov	BYTE PTR [rcx], al

; 123  : 			break;

	jmp	SHORT $LN4@PS2MouseHa
$LN9@PS2MouseHa:

; 124  : 		case 2:
; 125  : 			__ps2mouse->mouse_byte[2] = mouse_in;

	mov	eax, 1
	imul	rax, rax, 2
	mov	rcx, QWORD PTR ?__ps2mouse@@3PEAU__ps2mouse__@@EA ; __ps2mouse
	movzx	edx, BYTE PTR mouse_in$1[rbp]
	mov	BYTE PTR [rcx+rax+1], dl

; 126  : 			if (__ps2mouse->mouse_mode == MOUSE_SCROLLWHEEL){

	mov	rax, QWORD PTR ?__ps2mouse@@3PEAU__ps2mouse__@@EA ; __ps2mouse
	movzx	eax, BYTE PTR [rax+32]
	cmp	eax, 1
	jne	SHORT $LN10@PS2MouseHa

; 127  : 				++__ps2mouse->mouse_cycle;

	mov	rax, QWORD PTR ?__ps2mouse@@3PEAU__ps2mouse__@@EA ; __ps2mouse
	movzx	eax, BYTE PTR [rax]
	inc	al
	mov	rcx, QWORD PTR ?__ps2mouse@@3PEAU__ps2mouse__@@EA ; __ps2mouse
	mov	BYTE PTR [rcx], al

; 128  : 				break;

	jmp	SHORT $LN4@PS2MouseHa
$LN10@PS2MouseHa:

; 129  : 			}
; 130  : 			goto finish_packet;

	jmp	SHORT $LN11@PS2MouseHa
	jmp	SHORT $finish_packet$32
$LN12@PS2MouseHa:

; 131  : 		case 3:
; 132  : 			__ps2mouse->mouse_byte[3] = mouse_in;

	mov	eax, 1
	imul	rax, rax, 3
	mov	rcx, QWORD PTR ?__ps2mouse@@3PEAU__ps2mouse__@@EA ; __ps2mouse
	movzx	edx, BYTE PTR mouse_in$1[rbp]
	mov	BYTE PTR [rcx+rax+1], dl

; 133  : 			goto finish_packet;

	jmp	SHORT $LN13@PS2MouseHa
	jmp	SHORT $finish_packet$32
$LN14@PS2MouseHa:

; 134  : 		case 4:
; 135  : 			__ps2mouse->mouse_byte[4] = mouse_in;

	mov	eax, 1
	imul	rax, rax, 4
	mov	rcx, QWORD PTR ?__ps2mouse@@3PEAU__ps2mouse__@@EA ; __ps2mouse
	movzx	edx, BYTE PTR mouse_in$1[rbp]
	mov	BYTE PTR [rcx+rax+1], dl

; 136  : 			goto finish_packet;

	jmp	SHORT $LN15@PS2MouseHa
	jmp	SHORT $finish_packet$32
$LN4@PS2MouseHa:

; 137  : 		}
; 138  : 
; 139  : 		goto read_next;

	jmp	$LN16@PS2MouseHa
	jmp	$read_next$33
$LN11@PS2MouseHa:
$LN13@PS2MouseHa:
$LN15@PS2MouseHa:
$finish_packet$32:

; 140  : 
; 141  : 	finish_packet:
; 142  : 		__ps2mouse->mouse_cycle = 0;

	mov	rax, QWORD PTR ?__ps2mouse@@3PEAU__ps2mouse__@@EA ; __ps2mouse
	mov	BYTE PTR [rax], 0

; 143  : 
; 144  : 		int x = __ps2mouse->mouse_byte[1];

	mov	eax, 1
	imul	rax, rax, 1
	mov	rcx, QWORD PTR ?__ps2mouse@@3PEAU__ps2mouse__@@EA ; __ps2mouse
	movzx	eax, BYTE PTR [rcx+rax+1]
	mov	DWORD PTR x$2[rbp], eax

; 145  : 		int y = __ps2mouse->mouse_byte[2];

	mov	eax, 1
	imul	rax, rax, 2
	mov	rcx, QWORD PTR ?__ps2mouse@@3PEAU__ps2mouse__@@EA ; __ps2mouse
	movzx	eax, BYTE PTR [rcx+rax+1]
	mov	DWORD PTR y$3[rbp], eax

; 146  : 		if (x && __ps2mouse->mouse_byte[0] & (1 << 4))

	cmp	DWORD PTR x$2[rbp], 0
	je	SHORT $LN17@PS2MouseHa
	mov	eax, 1
	imul	rax, rax, 0
	mov	rcx, QWORD PTR ?__ps2mouse@@3PEAU__ps2mouse__@@EA ; __ps2mouse
	movzx	eax, BYTE PTR [rcx+rax+1]
	and	eax, 16
	test	eax, eax
	je	SHORT $LN17@PS2MouseHa

; 147  : 			x = x - 0x100;

	mov	eax, DWORD PTR x$2[rbp]
	sub	eax, 256				; 00000100H
	mov	DWORD PTR x$2[rbp], eax
$LN17@PS2MouseHa:

; 148  : 
; 149  : 		if (y && __ps2mouse->mouse_byte[0] & (1 << 5))

	cmp	DWORD PTR y$3[rbp], 0
	je	SHORT $LN18@PS2MouseHa
	mov	eax, 1
	imul	rax, rax, 0
	mov	rcx, QWORD PTR ?__ps2mouse@@3PEAU__ps2mouse__@@EA ; __ps2mouse
	movzx	eax, BYTE PTR [rcx+rax+1]
	and	eax, 32					; 00000020H
	test	eax, eax
	je	SHORT $LN18@PS2MouseHa

; 150  : 			y = y - 0x100;

	mov	eax, DWORD PTR y$3[rbp]
	sub	eax, 256				; 00000100H
	mov	DWORD PTR y$3[rbp], eax
$LN18@PS2MouseHa:

; 151  : 
; 152  : 		__ps2mouse->mouse_x_diff = x;

	mov	rax, QWORD PTR ?__ps2mouse@@3PEAU__ps2mouse__@@EA ; __ps2mouse
	mov	ecx, DWORD PTR x$2[rbp]
	mov	DWORD PTR [rax+14], ecx

; 153  : 		__ps2mouse->mouse_y_diff = y;

	mov	rax, QWORD PTR ?__ps2mouse@@3PEAU__ps2mouse__@@EA ; __ps2mouse
	mov	ecx, DWORD PTR y$3[rbp]
	mov	DWORD PTR [rax+18], ecx

; 154  : 		__ps2mouse->mouse_x += __ps2mouse->mouse_x_diff;

	mov	rax, QWORD PTR ?__ps2mouse@@3PEAU__ps2mouse__@@EA ; __ps2mouse
	mov	eax, DWORD PTR [rax+6]
	mov	rcx, QWORD PTR ?__ps2mouse@@3PEAU__ps2mouse__@@EA ; __ps2mouse
	add	eax, DWORD PTR [rcx+14]
	mov	rcx, QWORD PTR ?__ps2mouse@@3PEAU__ps2mouse__@@EA ; __ps2mouse
	mov	DWORD PTR [rcx+6], eax

; 155  : 		__ps2mouse->mouse_y -= __ps2mouse->mouse_y_diff;

	mov	rax, QWORD PTR ?__ps2mouse@@3PEAU__ps2mouse__@@EA ; __ps2mouse
	mov	rcx, QWORD PTR ?__ps2mouse@@3PEAU__ps2mouse__@@EA ; __ps2mouse
	mov	ecx, DWORD PTR [rcx+18]
	mov	eax, DWORD PTR [rax+10]
	sub	eax, ecx
	mov	rcx, QWORD PTR ?__ps2mouse@@3PEAU__ps2mouse__@@EA ; __ps2mouse
	mov	DWORD PTR [rcx+10], eax

; 156  : 
; 157  : 		if (__ps2mouse->mouse_x < 0)

	mov	rax, QWORD PTR ?__ps2mouse@@3PEAU__ps2mouse__@@EA ; __ps2mouse
	cmp	DWORD PTR [rax+6], 0
	jge	SHORT $LN19@PS2MouseHa

; 158  : 			__ps2mouse->mouse_x = 0;

	mov	rax, QWORD PTR ?__ps2mouse@@3PEAU__ps2mouse__@@EA ; __ps2mouse
	mov	DWORD PTR [rax+6], 0
$LN19@PS2MouseHa:

; 159  : 
; 160  : 		if (__ps2mouse->mouse_y < 0)

	mov	rax, QWORD PTR ?__ps2mouse@@3PEAU__ps2mouse__@@EA ; __ps2mouse
	cmp	DWORD PTR [rax+10], 0
	jge	SHORT $LN20@PS2MouseHa

; 161  : 			__ps2mouse->mouse_y = 0;

	mov	rax, QWORD PTR ?__ps2mouse@@3PEAU__ps2mouse__@@EA ; __ps2mouse
	mov	DWORD PTR [rax+10], 0
$LN20@PS2MouseHa:

; 162  : 
; 163  : 		__ps2mouse->mouse_butt_state = 0;

	mov	rax, QWORD PTR ?__ps2mouse@@3PEAU__ps2mouse__@@EA ; __ps2mouse
	mov	DWORD PTR [rax+28], 0

; 164  : 
; 165  : 		if (__ps2mouse->mouse_byte[0] & 0x01) {    //0x01 for PS/2

	mov	eax, 1
	imul	rax, rax, 0
	mov	rcx, QWORD PTR ?__ps2mouse@@3PEAU__ps2mouse__@@EA ; __ps2mouse
	movzx	eax, BYTE PTR [rcx+rax+1]
	and	eax, 1
	test	eax, eax
	je	SHORT $LN21@PS2MouseHa

; 166  : 			__ps2mouse->curr_button[0] = 1;

	mov	eax, 1
	imul	rax, rax, 0
	mov	rcx, QWORD PTR ?__ps2mouse@@3PEAU__ps2mouse__@@EA ; __ps2mouse
	mov	BYTE PTR [rcx+rax+25], 1

; 167  : 			__ps2mouse->mouse_butt_state |= LEFT_CLICK;

	mov	rax, QWORD PTR ?__ps2mouse@@3PEAU__ps2mouse__@@EA ; __ps2mouse
	mov	eax, DWORD PTR [rax+28]
	or	eax, 1
	mov	rcx, QWORD PTR ?__ps2mouse@@3PEAU__ps2mouse__@@EA ; __ps2mouse
	mov	DWORD PTR [rcx+28], eax

; 168  : 		}

	jmp	SHORT $LN22@PS2MouseHa
$LN21@PS2MouseHa:

; 169  : 		else
; 170  : 			__ps2mouse->curr_button[0] = 0;

	mov	eax, 1
	imul	rax, rax, 0
	mov	rcx, QWORD PTR ?__ps2mouse@@3PEAU__ps2mouse__@@EA ; __ps2mouse
	mov	BYTE PTR [rcx+rax+25], 0
$LN22@PS2MouseHa:

; 171  : 
; 172  : 		if (__ps2mouse->mouse_byte[0] & 0x02) {

	mov	eax, 1
	imul	rax, rax, 0
	mov	rcx, QWORD PTR ?__ps2mouse@@3PEAU__ps2mouse__@@EA ; __ps2mouse
	movzx	eax, BYTE PTR [rcx+rax+1]
	and	eax, 2
	test	eax, eax
	je	SHORT $LN23@PS2MouseHa

; 173  : 			__ps2mouse->curr_button[2] = 1;

	mov	eax, 1
	imul	rax, rax, 2
	mov	rcx, QWORD PTR ?__ps2mouse@@3PEAU__ps2mouse__@@EA ; __ps2mouse
	mov	BYTE PTR [rcx+rax+25], 1

; 174  : 			SeTextOut("Right clicked \r\n");

	lea	rcx, OFFSET FLAT:??_C@_0BB@LLAPAKF@Right?5clicked?5?$AN?6@
	call	SeTextOut

; 175  : 			__ps2mouse->mouse_butt_state |= RIGHT_CLICK;

	mov	rax, QWORD PTR ?__ps2mouse@@3PEAU__ps2mouse__@@EA ; __ps2mouse
	mov	eax, DWORD PTR [rax+28]
	or	eax, 2
	mov	rcx, QWORD PTR ?__ps2mouse@@3PEAU__ps2mouse__@@EA ; __ps2mouse
	mov	DWORD PTR [rcx+28], eax

; 176  : 		}

	jmp	SHORT $LN24@PS2MouseHa
$LN23@PS2MouseHa:

; 177  : 		else
; 178  : 			__ps2mouse->curr_button[2] = 0;

	mov	eax, 1
	imul	rax, rax, 2
	mov	rcx, QWORD PTR ?__ps2mouse@@3PEAU__ps2mouse__@@EA ; __ps2mouse
	mov	BYTE PTR [rcx+rax+25], 0
$LN24@PS2MouseHa:

; 179  : 
; 180  : 		if (__ps2mouse->mouse_byte[0] & 0x04)

	mov	eax, 1
	imul	rax, rax, 0
	mov	rcx, QWORD PTR ?__ps2mouse@@3PEAU__ps2mouse__@@EA ; __ps2mouse
	movzx	eax, BYTE PTR [rcx+rax+1]
	and	eax, 4
	test	eax, eax
	je	SHORT $LN25@PS2MouseHa

; 181  : 			__ps2mouse->mouse_butt_state |= MOUSE_MIDDLE_CLICK;

	mov	rax, QWORD PTR ?__ps2mouse@@3PEAU__ps2mouse__@@EA ; __ps2mouse
	mov	eax, DWORD PTR [rax+28]
	or	eax, 4
	mov	rcx, QWORD PTR ?__ps2mouse@@3PEAU__ps2mouse__@@EA ; __ps2mouse
	mov	DWORD PTR [rcx+28], eax
$LN25@PS2MouseHa:

; 182  : 
; 183  : 		if (__ps2mouse->mouse_byte[3]){

	mov	eax, 1
	imul	rax, rax, 3
	mov	rcx, QWORD PTR ?__ps2mouse@@3PEAU__ps2mouse__@@EA ; __ps2mouse
	movzx	eax, BYTE PTR [rcx+rax+1]
	test	eax, eax
	je	SHORT $LN26@PS2MouseHa

; 184  : 			if ((int8_t)__ps2mouse->mouse_byte[3] > 0)

	mov	eax, 1
	imul	rax, rax, 3
	mov	rcx, QWORD PTR ?__ps2mouse@@3PEAU__ps2mouse__@@EA ; __ps2mouse
	movsx	eax, BYTE PTR [rcx+rax+1]
	test	eax, eax
	jle	SHORT $LN27@PS2MouseHa

; 185  : 				__ps2mouse->mouse_butt_state |= MOUSE_SCROLL_DOWN;

	mov	rax, QWORD PTR ?__ps2mouse@@3PEAU__ps2mouse__@@EA ; __ps2mouse
	mov	eax, DWORD PTR [rax+28]
	or	eax, 6
	mov	rcx, QWORD PTR ?__ps2mouse@@3PEAU__ps2mouse__@@EA ; __ps2mouse
	mov	DWORD PTR [rcx+28], eax
	jmp	SHORT $LN28@PS2MouseHa
$LN27@PS2MouseHa:

; 186  : 			else if ((int8_t)__ps2mouse->mouse_byte[3] < 0)

	mov	eax, 1
	imul	rax, rax, 3
	mov	rcx, QWORD PTR ?__ps2mouse@@3PEAU__ps2mouse__@@EA ; __ps2mouse
	movsx	eax, BYTE PTR [rcx+rax+1]
	test	eax, eax
	jge	SHORT $LN29@PS2MouseHa

; 187  : 				__ps2mouse->mouse_butt_state |= MOUSE_SCROLL_UP;

	mov	rax, QWORD PTR ?__ps2mouse@@3PEAU__ps2mouse__@@EA ; __ps2mouse
	mov	eax, DWORD PTR [rax+28]
	or	eax, 5
	mov	rcx, QWORD PTR ?__ps2mouse@@3PEAU__ps2mouse__@@EA ; __ps2mouse
	mov	DWORD PTR [rcx+28], eax
$LN29@PS2MouseHa:
$LN28@PS2MouseHa:
$LN26@PS2MouseHa:

; 188  : 		}
; 189  : 
; 190  : 
; 191  : 		AuInputMessage newmsg;
; 192  : 		memset(&newmsg, 0, sizeof(AuInputMessage));

	mov	r8d, 26
	xor	edx, edx
	lea	rcx, QWORD PTR newmsg$4[rbp]
	call	memset

; 193  : 		newmsg.type = AU_INPUT_MOUSE;

	mov	BYTE PTR newmsg$4[rbp], 1

; 194  : 		newmsg.xpos = __ps2mouse->mouse_x;

	mov	rax, QWORD PTR ?__ps2mouse@@3PEAU__ps2mouse__@@EA ; __ps2mouse
	mov	eax, DWORD PTR [rax+6]
	mov	DWORD PTR newmsg$4[rbp+1], eax

; 195  : 		newmsg.ypos = __ps2mouse->mouse_y;

	mov	rax, QWORD PTR ?__ps2mouse@@3PEAU__ps2mouse__@@EA ; __ps2mouse
	mov	eax, DWORD PTR [rax+10]
	mov	DWORD PTR newmsg$4[rbp+5], eax

; 196  : 		newmsg.button_state = __ps2mouse->mouse_butt_state;

	mov	rax, QWORD PTR ?__ps2mouse@@3PEAU__ps2mouse__@@EA ; __ps2mouse
	movzx	eax, BYTE PTR [rax+28]
	mov	BYTE PTR newmsg$4[rbp+9], al

; 197  : 
; 198  : 		/*AuInputMessage oldmsg;
; 199  : 		AuDevReadMice(&oldmsg);*/
; 200  : 
; 201  : 		AuDevWriteMice(&newmsg);

	lea	rcx, QWORD PTR newmsg$4[rbp]
	call	AuDevWriteMice

; 202  : 		memcpy(__ps2mouse->prev_button, __ps2mouse->curr_button, 3);

	mov	rax, QWORD PTR ?__ps2mouse@@3PEAU__ps2mouse__@@EA ; __ps2mouse
	add	rax, 25
	mov	rcx, QWORD PTR ?__ps2mouse@@3PEAU__ps2mouse__@@EA ; __ps2mouse
	add	rcx, 22
	mov	r8d, 3
	mov	rdx, rax
	call	memcpy

; 203  : 		memset(__ps2mouse->curr_button, 0x00, 3);

	mov	rax, QWORD PTR ?__ps2mouse@@3PEAU__ps2mouse__@@EA ; __ps2mouse
	add	rax, 25
	mov	r8d, 3
	xor	edx, edx
	mov	rcx, rax
	call	memset
$LN16@PS2MouseHa:
$read_next$33:

; 204  : 
; 205  : 	read_next:
; 206  : 		break;

	jmp	SHORT $LN3@PS2MouseHa

; 207  : 	}

	jmp	$LN2@PS2MouseHa
$LN3@PS2MouseHa:

; 208  : 
; 209  : 	x64_sti();

	call	x64_sti

; 210  : 	AuInterruptEnd(12);

	mov	cl, 12
	call	AuInterruptEnd

; 211  : }

	lea	rsp, QWORD PTR [rbp+128]
	pop	rbp
	ret	0
?PS2MouseHandler@@YAX_KPEAX@Z ENDP			; PS2MouseHandler
_TEXT	ENDS
; Function compile flags: /Odtp /ZI
; File E:\Xeneva Project\Aurora\Kernel\Drivers\mouse.cpp
;	COMDAT ?AuPS2MouseRead@@YAEXZ
_TEXT	SEGMENT
?AuPS2MouseRead@@YAEXZ PROC				; AuPS2MouseRead, COMDAT

; 104  : uint8_t AuPS2MouseRead() {

$LN3:
	push	rbp
	sub	rsp, 96					; 00000060H
	lea	rbp, QWORD PTR [rsp+32]
	lea	rcx, OFFSET FLAT:__5B21646B_mouse@cpp
	call	__CheckForDebuggerJustMyCode

; 105  : 	PS2MouseWaitOutput();

	call	?PS2MouseWaitOutput@@YAXXZ		; PS2MouseWaitOutput

; 106  : 	return x64_inportb(0x60);

	mov	cx, 96					; 00000060H
	call	x64_inportb

; 107  : }

	lea	rsp, QWORD PTR [rbp+64]
	pop	rbp
	ret	0
?AuPS2MouseRead@@YAEXZ ENDP				; AuPS2MouseRead
_TEXT	ENDS
; Function compile flags: /Odtp /ZI
; File E:\Xeneva Project\Aurora\Kernel\Drivers\mouse.cpp
;	COMDAT ?AuPS2MouseWrite@@YAEE@Z
_TEXT	SEGMENT
write$ = 80
?AuPS2MouseWrite@@YAEE@Z PROC				; AuPS2MouseWrite, COMDAT

; 95   : uint8_t AuPS2MouseWrite(uint8_t write) {

$LN3:
	mov	BYTE PTR [rsp+8], cl
	push	rbp
	sub	rsp, 96					; 00000060H
	lea	rbp, QWORD PTR [rsp+32]
	lea	rcx, OFFSET FLAT:__5B21646B_mouse@cpp
	call	__CheckForDebuggerJustMyCode

; 96   : 	PS2MouseWaitInput();

	call	?PS2MouseWaitInput@@YAXXZ		; PS2MouseWaitInput

; 97   : 	x64_outportb(0x64, 0xD4);

	mov	dl, 212					; 000000d4H
	mov	cx, 100					; 00000064H
	call	x64_outportb

; 98   : 	PS2MouseWaitInput();

	call	?PS2MouseWaitInput@@YAXXZ		; PS2MouseWaitInput

; 99   : 	x64_outportb(0x60, write);

	movzx	edx, BYTE PTR write$[rbp]
	mov	cx, 96					; 00000060H
	call	x64_outportb

; 100  : 	PS2MouseWaitOutput();

	call	?PS2MouseWaitOutput@@YAXXZ		; PS2MouseWaitOutput

; 101  : 	return x64_inportb(0x60);

	mov	cx, 96					; 00000060H
	call	x64_inportb

; 102  : }

	lea	rsp, QWORD PTR [rbp+64]
	pop	rbp
	ret	0
?AuPS2MouseWrite@@YAEE@Z ENDP				; AuPS2MouseWrite
_TEXT	ENDS
; Function compile flags: /Odtp /ZI
; File E:\Xeneva Project\Aurora\Kernel\Drivers\mouse.cpp
;	COMDAT ?PS2MouseWaitOutput@@YAXXZ
_TEXT	SEGMENT
_timer_out$ = 0
?PS2MouseWaitOutput@@YAXXZ PROC				; PS2MouseWaitOutput, COMDAT

; 83   : void PS2MouseWaitOutput() {

$LN6:
	push	rbp
	sub	rsp, 112				; 00000070H
	lea	rbp, QWORD PTR [rsp+32]
	lea	rcx, OFFSET FLAT:__5B21646B_mouse@cpp
	call	__CheckForDebuggerJustMyCode

; 84   : 	uint32_t _timer_out = 100000;

	mov	DWORD PTR _timer_out$[rbp], 100000	; 000186a0H
$LN2@PS2MouseWa:

; 85   : 	while (--_timer_out) {

	mov	eax, DWORD PTR _timer_out$[rbp]
	dec	eax
	mov	DWORD PTR _timer_out$[rbp], eax
	cmp	DWORD PTR _timer_out$[rbp], 0
	je	SHORT $LN3@PS2MouseWa

; 86   : 		if (x64_inportb(0x64) & (1<<0))

	mov	cx, 100					; 00000064H
	call	x64_inportb
	movzx	eax, al
	and	eax, 1
	test	eax, eax
	je	SHORT $LN4@PS2MouseWa

; 87   : 			return;

	jmp	SHORT $LN1@PS2MouseWa
$LN4@PS2MouseWa:

; 88   : 	}

	jmp	SHORT $LN2@PS2MouseWa
$LN3@PS2MouseWa:
$LN1@PS2MouseWa:

; 89   : }

	lea	rsp, QWORD PTR [rbp+80]
	pop	rbp
	ret	0
?PS2MouseWaitOutput@@YAXXZ ENDP				; PS2MouseWaitOutput
_TEXT	ENDS
; Function compile flags: /Odtp /ZI
; File E:\Xeneva Project\Aurora\Kernel\Drivers\mouse.cpp
;	COMDAT ?PS2MouseWaitInput@@YAXXZ
_TEXT	SEGMENT
time_out$ = 0
?PS2MouseWaitInput@@YAXXZ PROC				; PS2MouseWaitInput, COMDAT

; 72   : void PS2MouseWaitInput() {

$LN6:
	push	rbp
	sub	rsp, 112				; 00000070H
	lea	rbp, QWORD PTR [rsp+32]
	lea	rcx, OFFSET FLAT:__5B21646B_mouse@cpp
	call	__CheckForDebuggerJustMyCode

; 73   : 	uint64_t time_out = 100000UL;

	mov	QWORD PTR time_out$[rbp], 100000	; 000186a0H
$LN2@PS2MouseWa:

; 74   : 	while (--time_out){

	mov	rax, QWORD PTR time_out$[rbp]
	dec	rax
	mov	QWORD PTR time_out$[rbp], rax
	cmp	QWORD PTR time_out$[rbp], 0
	je	SHORT $LN3@PS2MouseWa

; 75   : 		if (!(x64_inportb(0x64) & (1 << 1))) return;

	mov	cx, 100					; 00000064H
	call	x64_inportb
	movzx	eax, al
	and	eax, 2
	test	eax, eax
	jne	SHORT $LN4@PS2MouseWa
	jmp	SHORT $LN1@PS2MouseWa
$LN4@PS2MouseWa:

; 76   : 	}

	jmp	SHORT $LN2@PS2MouseWa
$LN3@PS2MouseWa:
$LN1@PS2MouseWa:

; 77   : }

	lea	rsp, QWORD PTR [rbp+80]
	pop	rbp
	ret	0
?PS2MouseWaitInput@@YAXXZ ENDP				; PS2MouseWaitInput
_TEXT	ENDS
; Function compile flags: /Odtp /ZI
; File E:\Xeneva Project\Aurora\Kernel\Drivers\mouse.cpp
;	COMDAT AuPS2DisableMouse
_TEXT	SEGMENT
AuPS2DisableMouse PROC					; COMDAT

; 227  : void AuPS2DisableMouse() {

$LN3:
	push	rbp
	sub	rsp, 96					; 00000060H
	lea	rbp, QWORD PTR [rsp+32]
	lea	rcx, OFFSET FLAT:__5B21646B_mouse@cpp
	call	__CheckForDebuggerJustMyCode

; 228  : 	x64_outportb(0x64, 0xA7);

	mov	dl, 167					; 000000a7H
	mov	cx, 100					; 00000064H
	call	x64_outportb

; 229  : 	x64_outportb(0x64, 0xAD);

	mov	dl, 173					; 000000adH
	mov	cx, 100					; 00000064H
	call	x64_outportb

; 230  : }

	lea	rsp, QWORD PTR [rbp+64]
	pop	rbp
	ret	0
AuPS2DisableMouse ENDP
_TEXT	ENDS
; Function compile flags: /Odtp /ZI
; File E:\Xeneva Project\Aurora\Kernel\Drivers\mouse.cpp
;	COMDAT ?AuPS2MouseSetPos@@YAXHH@Z
_TEXT	SEGMENT
x$ = 80
y$ = 88
?AuPS2MouseSetPos@@YAXHH@Z PROC				; AuPS2MouseSetPos, COMDAT

; 220  : void AuPS2MouseSetPos(int32_t x, int32_t y) {

$LN4:
	mov	DWORD PTR [rsp+16], edx
	mov	DWORD PTR [rsp+8], ecx
	push	rbp
	sub	rsp, 96					; 00000060H
	lea	rbp, QWORD PTR [rsp+32]
	lea	rcx, OFFSET FLAT:__5B21646B_mouse@cpp
	call	__CheckForDebuggerJustMyCode

; 221  : 	if (!__ps2mouse)

	cmp	QWORD PTR ?__ps2mouse@@3PEAU__ps2mouse__@@EA, 0 ; __ps2mouse
	jne	SHORT $LN2@AuPS2Mouse

; 222  : 		return;

	jmp	SHORT $LN1@AuPS2Mouse
$LN2@AuPS2Mouse:

; 223  : 	__ps2mouse->mouse_x = x;

	mov	rax, QWORD PTR ?__ps2mouse@@3PEAU__ps2mouse__@@EA ; __ps2mouse
	mov	ecx, DWORD PTR x$[rbp]
	mov	DWORD PTR [rax+6], ecx

; 224  : 	__ps2mouse->mouse_y = y;

	mov	rax, QWORD PTR ?__ps2mouse@@3PEAU__ps2mouse__@@EA ; __ps2mouse
	mov	ecx, DWORD PTR y$[rbp]
	mov	DWORD PTR [rax+10], ecx
$LN1@AuPS2Mouse:

; 225  : }

	lea	rsp, QWORD PTR [rbp+64]
	pop	rbp
	ret	0
?AuPS2MouseSetPos@@YAXHH@Z ENDP				; AuPS2MouseSetPos
_TEXT	ENDS
; Function compile flags: /Odtp /ZI
; File E:\Xeneva Project\Aurora\Kernel\Drivers\mouse.cpp
;	COMDAT ?AuPS2MouseInitialise@@YAXXZ
_TEXT	SEGMENT
status$ = 0
?AuPS2MouseInitialise@@YAXXZ PROC			; AuPS2MouseInitialise, COMDAT

; 235  : void AuPS2MouseInitialise() {

$LN4:
	push	rbp
	sub	rsp, 112				; 00000070H
	lea	rbp, QWORD PTR [rsp+32]
	lea	rcx, OFFSET FLAT:__5B21646B_mouse@cpp
	call	__CheckForDebuggerJustMyCode

; 236  : 	__ps2mouse = (PS2Mouse*)kmalloc(sizeof(PS2Mouse));

	mov	ecx, 37					; 00000025H
	call	kmalloc
	mov	QWORD PTR ?__ps2mouse@@3PEAU__ps2mouse__@@EA, rax ; __ps2mouse

; 237  : 	memset(__ps2mouse, 0, sizeof(PS2Mouse));

	mov	r8d, 37					; 00000025H
	xor	edx, edx
	mov	rcx, QWORD PTR ?__ps2mouse@@3PEAU__ps2mouse__@@EA ; __ps2mouse
	call	memset

; 238  : 
; 239  : 	__ps2mouse->mouse_mode = MOUSE_DEFAULT;

	mov	rax, QWORD PTR ?__ps2mouse@@3PEAU__ps2mouse__@@EA ; __ps2mouse
	mov	BYTE PTR [rax+32], 0

; 240  : 	uint8_t status;
; 241  : 
; 242  : 	PS2MouseWaitInput();

	call	?PS2MouseWaitInput@@YAXXZ		; PS2MouseWaitInput

; 243  : 	x64_outportb(0x64, 0xA8);

	mov	dl, 168					; 000000a8H
	mov	cx, 100					; 00000064H
	call	x64_outportb

; 244  : 
; 245  : 	PS2MouseWaitInput();

	call	?PS2MouseWaitInput@@YAXXZ		; PS2MouseWaitInput

; 246  : 	x64_outportb(0x64, 0x20);

	mov	dl, 32					; 00000020H
	mov	cx, 100					; 00000064H
	call	x64_outportb

; 247  : 
; 248  : 	PS2MouseWaitInput();

	call	?PS2MouseWaitInput@@YAXXZ		; PS2MouseWaitInput

; 249  : 	status = (x64_inportb(0x60) | 2);

	mov	cx, 96					; 00000060H
	call	x64_inportb
	movzx	eax, al
	or	eax, 2
	mov	BYTE PTR status$[rbp], al

; 250  : 
; 251  : 	PS2MouseWaitInput();

	call	?PS2MouseWaitInput@@YAXXZ		; PS2MouseWaitInput

; 252  : 	x64_outportb(0x64, 0x60);

	mov	dl, 96					; 00000060H
	mov	cx, 100					; 00000064H
	call	x64_outportb

; 253  : 
; 254  : 	PS2MouseWaitInput();

	call	?PS2MouseWaitInput@@YAXXZ		; PS2MouseWaitInput

; 255  : 	x64_outportb(0x60, status);

	movzx	edx, BYTE PTR status$[rbp]
	mov	cx, 96					; 00000060H
	call	x64_outportb

; 256  : 
; 257  : 	AuPS2MouseWrite(0xF6);

	mov	cl, 246					; 000000f6H
	call	?AuPS2MouseWrite@@YAEE@Z		; AuPS2MouseWrite

; 258  : 	AuPS2MouseRead();

	call	?AuPS2MouseRead@@YAEXZ			; AuPS2MouseRead

; 259  : 
; 260  : 	AuPS2MouseWrite(0xF4);

	mov	cl, 244					; 000000f4H
	call	?AuPS2MouseWrite@@YAEE@Z		; AuPS2MouseWrite

; 261  : 	AuPS2MouseRead();

	call	?AuPS2MouseRead@@YAEXZ			; AuPS2MouseRead

; 262  : 
; 263  : 	/* Enable the scroll wheel */
; 264  : 	AuPS2MouseWrite(MOUSE_DEVICE_ID);

	mov	cl, 242					; 000000f2H
	call	?AuPS2MouseWrite@@YAEE@Z		; AuPS2MouseWrite

; 265  : 	status = AuPS2MouseRead();

	call	?AuPS2MouseRead@@YAEXZ			; AuPS2MouseRead
	mov	BYTE PTR status$[rbp], al

; 266  : 	AuPS2MouseWrite(MOUSE_SAMPLE_RATE);

	mov	cl, 243					; 000000f3H
	call	?AuPS2MouseWrite@@YAEE@Z		; AuPS2MouseWrite

; 267  : 	AuPS2MouseWrite(200);

	mov	cl, 200					; 000000c8H
	call	?AuPS2MouseWrite@@YAEE@Z		; AuPS2MouseWrite

; 268  : 	AuPS2MouseWrite(MOUSE_SAMPLE_RATE);

	mov	cl, 243					; 000000f3H
	call	?AuPS2MouseWrite@@YAEE@Z		; AuPS2MouseWrite

; 269  : 	AuPS2MouseWrite(100);

	mov	cl, 100					; 00000064H
	call	?AuPS2MouseWrite@@YAEE@Z		; AuPS2MouseWrite

; 270  : 	AuPS2MouseWrite(MOUSE_SAMPLE_RATE);

	mov	cl, 243					; 000000f3H
	call	?AuPS2MouseWrite@@YAEE@Z		; AuPS2MouseWrite

; 271  : 	AuPS2MouseWrite(80);

	mov	cl, 80					; 00000050H
	call	?AuPS2MouseWrite@@YAEE@Z		; AuPS2MouseWrite

; 272  : 	AuPS2MouseWrite(MOUSE_DEVICE_ID);

	mov	cl, 242					; 000000f2H
	call	?AuPS2MouseWrite@@YAEE@Z		; AuPS2MouseWrite

; 273  : 	status = AuPS2MouseRead();

	call	?AuPS2MouseRead@@YAEXZ			; AuPS2MouseRead
	mov	BYTE PTR status$[rbp], al

; 274  : 	if (status == 3) {

	movzx	eax, BYTE PTR status$[rbp]
	cmp	eax, 3
	jne	SHORT $LN2@AuPS2Mouse

; 275  : 		AuTextOut("PS2 Mouse with scroll wheel \n");

	lea	rcx, OFFSET FLAT:??_C@_0BO@EPKMAGCI@PS2?5Mouse?5with?5scroll?5wheel?5?6@
	call	AuTextOut

; 276  : 		__ps2mouse->mouse_mode = MOUSE_SCROLLWHEEL;

	mov	rax, QWORD PTR ?__ps2mouse@@3PEAU__ps2mouse__@@EA ; __ps2mouse
	mov	BYTE PTR [rax+32], 1
$LN2@AuPS2Mouse:

; 277  : 	}
; 278  : 
; 279  : 	__ps2mouse->mouse_x = 0;

	mov	rax, QWORD PTR ?__ps2mouse@@3PEAU__ps2mouse__@@EA ; __ps2mouse
	mov	DWORD PTR [rax+6], 0

; 280  : 	__ps2mouse->mouse_y = 0;

	mov	rax, QWORD PTR ?__ps2mouse@@3PEAU__ps2mouse__@@EA ; __ps2mouse
	mov	DWORD PTR [rax+10], 0

; 281  : 
; 282  : 	AuHalRegisterIRQ(34, PS2MouseHandler, 12, false);  //34

	xor	r9d, r9d
	mov	r8b, 12
	lea	rdx, OFFSET FLAT:?PS2MouseHandler@@YAX_KPEAX@Z ; PS2MouseHandler
	mov	ecx, 34					; 00000022H
	call	AuHalRegisterIRQ

; 283  : }

	lea	rsp, QWORD PTR [rbp+80]
	pop	rbp
	ret	0
?AuPS2MouseInitialise@@YAXXZ ENDP			; AuPS2MouseInitialise
_TEXT	ENDS
END
