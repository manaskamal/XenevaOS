...
;	COMDAT pdata
pdata	SEGMENT
$pdata$?ProcessSleep@@YAH_K@Z DD imagerel $LN6
	DD	imagerel $LN6+121
...
; 337  : int GetCurrentTime(void* ptr) {
...
; 339  : 	AuGetCurrentTime((AuTime*)ptr);
...
; 320  : int SendSignal(int pid,int signo) {
...
; 322  : 	AuProcess* proc = AuProcessFindPID(pid);
...
; 325  : 
; 326  : 	AuThread* mainthr = proc->main_thread;
...
; 329  : 	AuSendSignal(mainthr->id, signo);
...
; 268  : int SetFileToProcess(int fileno, int dest_fdidx, int proc_id) {
...
; 270  : 	AuThread* thr = AuGetCurrentThread();
...
; 273  : 	/* file check if current thread's process is
; 274  : 	 * found by checking twice, first by
; 275  : 	 * main thread checkup second by sub thread
; 276  : 	 * checkup
; 277  : 	 */
; 278  : 	AuProcess* proc = AuProcessFindThread(thr);
...
; 280  : 		proc = AuProcessFindSubThread(thr);
...
; 283  : 	}
; 284  : 
; 285  : 	/* now try getting the destination process by its
; 286  : 	* process id
; 287  : 	*/
; 288  : 	AuProcess* destproc = AuProcessFindPID(proc_id);
...
; 291  : 
; 292  : 	/* now try getting the file from current process
; 293  : 	 * file entry
; 294  : 	 */
; 295  : 	AuVFSNode* file = proc->fds[fileno];
...
; 298  : 
; 299  : 	AuVFSNode *destfile = destproc->fds[dest_fdidx];
...
; 302  : 	else {
; 303  : 		/* now we have no file entry in destination
; 304  : 		 * process's file index, so make entry
; 305  : 		 * of current process's file targeted by
; 306  : 		 * fileno to destination processes file
; 307  : 		 * entry 
; 308  : 		 */
; 309  : 		destproc->fds[dest_fdidx] = file;
...
; 250  : int CloseUserThread(int thread_idx) {
...
; 255  : 	AuProcess* proc = AuProcessFindThread(thr);
...
; 227  : int CreateUserThread(void(*entry) (void*), char *name){
...
; 229  : 	AuThread* thr = AuGetCurrentThread();
...
; 232  : 	AuProcess* proc = AuProcessFindThread(thr);
...
; 234  : 		proc = AuProcessFindSubThread(thr);
...
; 237  : 	}
; 238  : 	int idx = AuCreateUserthread(proc, entry, name);
...
; 219  : 	return AuGetSystemTimerTick();
...
; 207  : int SetSignal(int signo, AuSigHandler handler){
...
; 178  : int ProcessSleep(uint64_t ms) {
...
; 180  : 	if (ms <= 0)
	...
	cmp	QWORD PTR ms$[rbp], 0
	ja	SHORT $LN2@ProcessSle
...
; 182  : 
; 183  : 	uint64_t sleep_time = ms * 1000;

	imul	rax, QWORD PTR ms$[rbp], 1000		; 000003e8H
	mov	QWORD PTR sleep_time$[rbp], rax

; 184  : 	AuThread* current_thr = AuGetCurrentThread();
...
; 187  : 	if (current_thr->pendingSigCount > 0)

	mov	rax, QWORD PTR current_thr$[rbp]
	movzx	eax, BYTE PTR [rax+632]
	test	eax, eax
	jle	SHORT $LN4@ProcessSle
...
; 160  : 	}
; 161  : 	
; 162  : 	int status = AuLoadExecToProcess(proc, filename, argc,allocated_argv);
