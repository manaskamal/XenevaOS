...
PUBLIC	??_C@_0EM@JFNPENHO@?$FLAurora?$FN?3?5Font?5Manager?5failed?5t@ ; `string'
PUBLIC	??_C@_0BD@HBHONKLG@Font?5Count?5?9?$DO?$CFd?5?$AN?6@ ; `string'
EXTRN	strcmp:PROC
EXTRN	strcpy:PROC
EXTRN	strchr:PROC
EXTRN	memset:PROC
...
;	COMDAT pdata
pdata	SEGMENT
$pdata$?FontManagerInitialise@@YAXXZ DD imagerel $LN12
...
pdata	ENDS
;	COMDAT ??_C@_0BD@HBHONKLG@Font?5Count?5?9?$DO?$CFd?5?$AN?6@
CONST	SEGMENT
??_C@_0BD@HBHONKLG@Font?5Count?5?9?$DO?$CFd?5?$AN?6@ DB 'Font Count ->%d '
	DB	0dH, 0aH, 00H				; `string'
CONST	ENDS
;	COMDAT ??_C@_0EM@JFNPENHO@?$FLAurora?$FN?3?5Font?5Manager?5failed?5t@
CONST	SEGMENT
??_C@_0EM@JFNPENHO@?$FLAurora?$FN?3?5Font?5Manager?5failed?5t@ DB '[Auror'
	DB	'a]: Font Manager failed to open ftlst.cnf, ftlst.cnf file not'
...
; 261  : int AuFTMngrGetFontSize(char* fontname) {
...
; 262  : 	for (FontSeg* seg = firstSeg; seg != NULL; seg = seg->next) {
...
; 263  : 		if (strcmp(fontname, seg->fontname) == 0) {
...
; 246  : int AuFTMngrGetFontID(char* fontname) {
...
; 248  : 	for (FontSeg* seg = firstSeg; seg != NULL; seg = seg->next) {
...
; 249  : 		if (strcmp(fontname, seg->fontname) == 0) {
...
; 250  : 			font_id =  (seg->sharedSeg->id << 16) | seg->sharedSeg->key & UINT16_MAX;
...
; 217  : 	if (!fontconf) {
	...
	jne	SHORT $LN8@FontManage

; 218  : 		AuTextOut("[Aurora]: Font Manager failed to open ftlst.cnf, ftlst.cnf file not found \n");

	lea	rcx, OFFSET FLAT:??_C@_0EM@JFNPENHO@?$FLAurora?$FN?3?5Font?5Manager?5failed?5t@
	call	AuTextOut
$LN2@FontManage:
...
; 221  : 	}
; 222  : 	int num_pages = fontconf->size / PAGE_SIZE;
...
; 225  : 
; 226  : 	uint64_t* first_addr = NULL;

	mov	QWORD PTR first_addr$[rbp], 0

; 227  : 	for (int i = 0; i < num_pages; i++) {
...
; 228  : 		uint64_t* addr = (uint64_t*)P2V((size_t)AuPmmngrAlloc());
...
; 232  : 	
; 233  : 	memset(first_addr, 0, fontconf->size);
...
; 234  : 	AuVFSNodeRead(fs, fontconf, first_addr, fontconf->size);
...
; 236  : 	font_conf_data = (uint8_t*)first_addr;

	mov	rax, QWORD PTR first_addr$[rbp]
	mov	QWORD PTR ?font_conf_data@@3PEAEEA, rax	; font_conf_data

; 237  : 	totalSysFonts = FontManagerGetFontCount(font_conf_data);
...
; 238  : 	SeTextOut("Font Count ->%d \r\n", totalSysFonts);

	mov	edx, DWORD PTR ?totalSysFonts@@3HA	; totalSysFonts
	lea	rcx, OFFSET FLAT:??_C@_0BD@HBHONKLG@Font?5Count?5?9?$DO?$CFd?5?$AN?6@
	call	SeTextOut

; 239  : 	FontManagerIterateFontList(font_conf_data);
