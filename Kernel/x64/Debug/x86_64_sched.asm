...
; 430  : extern "C" void AuPrintStack() {
...
; 374  : void x8664SchedulerISR(size_t v, void* param) {
...
; 376  : 	interrupt_stack_frame *frame = (interrupt_stack_frame*)param;

	mov	rax, QWORD PTR param$[rbp]
	mov	QWORD PTR frame$[rbp], rax

; 377  : 	if (_x86_64_sched_enable == false)
...
; 383  : 		current_thread->frame.cr3 = x64_read_cr3();

	call	x64_read_cr3
	mov	rcx, QWORD PTR current_thread$[rbp]
	mov	QWORD PTR [rcx+192], rax

; 384  : 		current_thread->frame.kern_esp = x64_get_kstack(ktss);
...
; 385  : 		/* check for any signal */
; 386  : 		if (AuCheckSignal(current_thread, frame)) {
...
; 389  : 
; 390  : 			/* actually, signal should be executed immediately
; 391  : 			 * here, AuSignalHandle() 
; 392  : 			 */
; 393  : 		}
; 394  : 
; 395  : 		if (x86_64_is_cpu_fxsave_supported())
...
; 396  : 			x64_fxsave(current_thread->fx_state);
...
; 400  : 		current_thread = AuPerCPUGetCurrentThread();

	call	?AuPerCPUGetCurrentThread@@YAPEAU_au_thread_@@XZ ; AuPerCPUGetCurrentThread
	mov	QWORD PTR current_thread$[rbp], rax

; 401  : 		
; 402  : 		AuInterruptEnd(0);

	xor	ecx, ecx
	call	AuInterruptEnd

; 403  : 		
; 404  : 		if (x86_64_is_cpu_fxsave_supported())
...
; 405  : 			x64_fxrstor(current_thread->fx_state);
...
; 406  : 
; 407  : 		x64_set_kstack(ktss, current_thread->frame.kern_esp);
...
; 408  : 
; 409  : 		x64_ldmxcsr(&current_thread->mxcsr);
...
; 410  : 	
; 411  : 		execute_idle(current_thread, ktss);
...
; 344  : 			if (thread->quanta <= 0) 

	mov	rax, QWORD PTR thread$[rbp]
	cmp	QWORD PTR [rax+303], 0
	ja	SHORT $LN6@AuNextThre

; 345  : 				thread->state = THREAD_STATE_READY;
...
; 362  : 	if (_run_idle)
	...
	test	eax, eax
	je	SHORT $LN10@AuNextThre

; 363  : 		thread = _idle_thr;
...
; 364  : 
; 365  : 	//current_thread = thread;
; 366  : 	AuPerCPUSetCurrentThread(thread);
...
; 561  : uint64_t AuGetSystemTimerTick() {
...
; 553  : bool AuIsSchedulerInitialised() {
...
; 548  : AU_EXTERN AU_EXPORT void AuForceScheduler() {
...
; 536  : AuThread* AuThreadFindByIDBlockList(uint16_t id){
...
; 537  : 	for (AuThread* block_queue = blocked_thr_head; block_queue != NULL; block_queue = block_queue->next){
...
; 523  : AuThread* AuThreadFindByID(uint16_t id) {
...
; 524  : 	for (AuThread* ready_queue_ = thread_list_head; ready_queue_ != NULL; ready_queue_ = ready_queue_->next) {
...
; 514  : void AuThreadCleanTrash(AuThread* t) {
...
; 487  : void AuThreadMoveToTrash(AuThread* t) {
...
; 490  : 
; 491  : 	t->state = THREAD_STATE_KILLABLE;

	mov	rax, QWORD PTR t$[rbp]
	mov	BYTE PTR [rax+300], 5

; 492  : 
; 493  : 	/* search the thread in ready queue*/
; 494  : 	for (AuThread* ready_queue_ = thread_list_head; ready_queue_ != NULL; ready_queue_ = ready_queue_->next) {
...
; 498  : 
; 499  : 	/* search the thread in block queue*/
; 500  : 	for (AuThread* block_queue_ = blocked_thr_head; block_queue_ != NULL; block_queue_ = block_queue_->next) {
...
; 504  : 
; 505  : 	/* insert it in the trash list */
; 506  : 	AuThreadInsertTrash(t);
...
; 468  : AU_EXTERN AU_EXPORT void AuUnblockThread(AuThread *t) {
...
; 470  : 	bool found_ = false;

	mov	BYTE PTR found_$[rbp], 0

; 471  : 	for (AuThread *thr = blocked_thr_head; thr != NULL; thr = thr->next) {
...
; 457  : AU_EXTERN AU_EXPORT void AuSleepThread(AuThread *thread, uint64_t ms) {
...
; 458  : 	thread->state = THREAD_STATE_SLEEP;

	mov	rax, QWORD PTR thread$[rbp]
	mov	BYTE PTR [rax+300], 4

; 459  : 	thread->quanta = ms;
...
; 448  : AU_EXTERN AU_EXPORT void AuBlockThread(AuThread *thread) {
...
; 449  : 	thread->state = THREAD_STATE_BLOCKED;

	mov	rax, QWORD PTR thread$[rbp]
	mov	BYTE PTR [rax+300], 3

; 450  : 	AuThreadDelete(thread);

	mov	rcx, QWORD PTR thread$[rbp]
	call	?AuThreadDelete@@YAXPEAU_au_thread_@@@Z	; AuThreadDelete

; 451  : 	AuThreadInsertBlock(thread);
...
; 439  : AuThread* AuGetCurrentThread() {
...
; 440  : 	return AuPerCPUGetCurrentThread();
...
; 422  : 	_x86_64_sched_init = true;

	mov	BYTE PTR ?_x86_64_sched_init@@3_NA, 1	; _x86_64_sched_init

; 423  : 	setvect(0x40, x8664SchedulerISR);  //0x40

	lea	rdx, OFFSET FLAT:?x8664SchedulerISR@@YAX_KPEAX@Z ; x8664SchedulerISR
	mov	ecx, 64					; 00000040H
	call	setvect

; 424  : 	AuThread* current_thread = AuPerCPUGetCurrentThread();

	call	?AuPerCPUGetCurrentThread@@YAPEAU_au_thread_@@XZ ; AuPerCPUGetCurrentThread
	mov	QWORD PTR current_thread$[rbp], rax

; 425  : 	execute_idle(current_thread, x86_64_get_tss());
