...
; 63   : AuVFSNode* AuVFSFind(char* path) {
...
; 64   : 
; 65   : 	AuVFSNode* Returnable = NULL;

	mov	QWORD PTR Returnable$[rsp], 0

; 66   : 	/* first of all search all file system 
; 67   : 	 * skipping '/' of the path 
; 68   : 	 */
; 69   : 	char* next = strchr(path, '/');
...
; 81   : 
; 82   : 	for (int j = 0; j < __RootContainer->childs->pointer; j++) {
...
; 83   : 		AuVFSNode* node = (AuVFSNode*)list_get_at(__RootContainer->childs, j);
...
; 84   : 		if ((strcmp(node->filename, pathname) == 0) && (node->flags & FS_FLAG_FILE_SYSTEM)) {
...
; 117  : void AuVFSRegisterRoot(AuVFSNode *fs) {
...
; 102  : AU_EXTERN AU_EXPORT void AuVFSAddFileSystem(AuVFSNode* node) {

$LN7:
	mov	QWORD PTR [rsp+8], rcx
	sub	rsp, 56					; 00000038H

; 103  : 	for (int i = 0; i < __RootContainer->childs->pointer; i++) {
...
; 104  : 		AuVFSNode* node_ = (AuVFSNode*)list_get_at(__RootContainer->childs, i);
...
; 108  : 	list_add(__RootContainer->childs, node);
...
; 175  : AU_EXTERN AU_EXPORT void AuVFSNodeIOControl(AuVFSNode* node, int code, void* arg) {
...
; 177  : 		node->iocontrol(node, code, arg);
...
; 128  : AU_EXTERN AU_EXPORT AuVFSNode* AuVFSOpen(char* path){

$LN16:
	mov	QWORD PTR [rsp+8], rcx
	sub	rsp, 104				; 00000068H

; 129  : 	AuVFSNode *Returnable = NULL;

	mov	QWORD PTR Returnable$[rsp], 0

; 130  : 	AuVFSNode* fs = AuVFSFind(path);
...
; 135  : 		
; 136  : 		/* just skip the '/' from the path */
; 137  : 		char* next = strchr(path, '/');
...
; 142  : 			Returnable = fs->open(fs, path);
...
; 157  : 
; 158  : 		/* skip the fs filename, from the path
; 159  : 		 * and just pass the required path */
; 160  : 		if (strcmp(fs->filename, pathname) == 0) 
...
; 163  : 			Returnable = fs->open(fs,next);
...
; 44   : void AuVFSInitialise() {

$LN3:
	sub	rsp, 56					; 00000038H

; 45   : 	AuVFSContainer* _root = (AuVFSContainer*)kmalloc(sizeof(AuVFSContainer));

	mov	ecx, 8
	call	kmalloc
	mov	QWORD PTR _root$[rsp], rax

; 46   : 	_root->childs = initialize_list();

	call	initialize_list
	mov	rcx, QWORD PTR _root$[rsp]
	mov	QWORD PTR [rcx], rax

; 47   : 	__RootContainer = _root;
...
; 49   : 
; 50   : 	/* initialise the device file system */
; 51   : 	AuDeviceFsInitialize();

	call	?AuDeviceFsInitialize@@YAXXZ		; AuDeviceFsInitialize

; 52   : 	/* here we need to mount the
; 53   : 	 * root file system
; 54   : 	 */
; 55   : }
