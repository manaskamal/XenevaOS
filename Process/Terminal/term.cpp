...
int main(int argc, char* arv[]){
	...
	win = ChCreateWindow(app, (1 << 0), "Xeneva Terminal", 300, 300, CHITRALEKHA_DEFAULT_WIN_WIDTH, CHITRALEKHA_DEFAULT_WIN_HEIGHT);
	win->color = BLACK;
	ChWindowPaint(win);
	
	ChFontDrawText(win->canv, app->baseFont, "Happy Navratri !!", 10, 40, 10, WHITE);
	ChWindowUpdate(win, 0, 0, win->info->width, win->info->height,0, 1);
	PostEvent e;
	memset(&e, 0, sizeof(PostEvent));
