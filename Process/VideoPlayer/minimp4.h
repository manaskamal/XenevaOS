...
typedef enum { e_audio, e_video, e_private } track_media_kind_t;
...
*/
MP4E_mux_t*
MP4E_open(int sequential_mode_flag,
...
*/
int MP4E_put_sample(
	MP4E_mux_t* mux, int track_num, const void* data, int data_bytes, int duration, int kind);
...
enum {
	...
	BOX_stik = FOUR_CHAR_INT(
		's',
		't',
		'i',
		'k'), // stik (byte)  0 = Movie   1 = Normal  2 = Audiobook  5 = Whacked Bookmark  6 = Music Video  9 = Short Film  10 = TV Show  11 = Booklet  14 = Ringtone
...
#else
	// as in ffmpeg
	0,	 0,	  0,   0x20, 'f', 't', 'y', 'p', 'i', 's', 'o', 'm', 0,	  0,   2,	0,
	'm', 'p', '4', '1',	 'i', 's', 'o', 'm', 'i', 's', 'o', '2', 'a', 'v', 'c', '1',
#endif
};
...
MP4E_open(int sequential_mode_flag,
	...
	if (!mux->sequential_mode_flag) { // Write filler, which would be updated later
...
int MP4E_add_track(MP4E_mux_t* mux, const MP4E_track_t* track_data) {
	...
	memcpy(&tr->info, (void*)track_data, sizeof(*track_data));
...
int MP4E_set_dsi(MP4E_mux_t* mux, int track_id, const void* dsi, int bytes) {
	...
	assert(tr->info.track_media_kind == e_audio || tr->info.track_media_kind == e_private);
	if (tr->vsps.bytes)
...
static int write_pending_data(MP4E_mux_t* mux, track_t* tr) {
	...
	if (tr->pending_sample.bytes > 0 && tr->smpl.bytes >= sizeof(sample_t)) {
		...
		// Write data
		ERR(mux->write_callback(
			mux->write_pos, tr->pending_sample.data, tr->pending_sample.bytes, mux->token));
...
static int
add_sample_descriptor(MP4E_mux_t* mux, track_t* tr, int data_bytes, int duration, int kind) {
	sample_t smp;
	smp.size = data_bytes;
...
*/
static int mp4e_write_fragment_header(MP4E_mux_t* mux,
									  int track_num,
									  int data_bytes,
									  int duration,
									  int kind
...
#define MOOV_TIMESCALE 1000
	...
	for (ntr = 0; ntr < ntracks; ntr++) {
		...
		WRITE_4(duration); // duration
		{
			int lang_code = ((tr->info.language[0] & 31) << 10) |
							((tr->info.language[1] & 31) << 5) | (tr->info.language[2] & 31);
...
#define WRITE_OD_LEN(size)                                                                         \
	...
	WRITE_1(size)
				...
				WRITE_3(tr->info.u.a.channelcount * 6144 /
						8); // bufferSizeDB in bytes, constant as in reference decoder
		...
		if (tr->info.track_media_kind == e_video &&
			(MP4_OBJECT_TYPE_AVC == tr->info.object_type_indication ||
			...
			if (MP4_OBJECT_TYPE_AVC == tr->info.object_type_indication) {
				...
				WRITE_2(
					0); // progressive, interlaced, non packed constraint, frame only constraint flags
				...
				WRITE_1(
					3); // ConstantFrameRate (2), NumTemporalLayers (3), TemporalIdNested (1), LengthSizeMinusOne (2)
		...
		WRITE_4(
			0); // sample_size  If this field is set to 0, then the samples have different sizes, and those sizes
...
#define MP4E_HANDLER_TYPE_MDIR 0x6d646972
		...
		WRITE_4(0); // reserved[3]
		WRITE_4(
			0); // name is a null-terminated string in UTF-8 characters which gives a human-readable name for the track type (for debugging and inspection purposes).
		END_ATOM;
		ATOM(BOX_ilst);
		ATOM(BOX_ccmt);
		ATOM(BOX_data);
...
#define SWAP32(x)                                                                                  \
	(uint32_t)((((x) >> 24) & 0xFF) | (((x) >> 8) & 0xFF00) | (((x) << 8) & 0xFF0000) |            \
			   ((x & 0xFF) << 24))
...
static int
patch_pps(h264_sps_id_patcher_t* h, bit_reader_t* bs, bs_t* bd, int new_pps_id, int* old_id) {
	int bytes;
...
static int transcode_nalu(h264_sps_id_patcher_t* h,
	...
	*dst = *src;
	...
	case 5:
		...
		break;
	default:
		memcpy(dst, (void*)src, nalu_bytes);
		return nalu_bytes;
	}
...
find_nal_unit(const uint8_t* h264_data, int h264_data_bytes, int* pnal_unit_bytes) {
	const uint8_t* eof = h264_data + h264_data_bytes;
	int zcount;
...
static int mp4_h265_write_nal(mp4_h26x_writer_t* h,
							  const unsigned char* nal,
							  int sizeof_nal,
							  unsigned timeStamp90kHz_next) {
	...
	default:
			...
			return MP4E_STATUS_BAD_ARGUMENTS;
		{
			...
			memcpy(tmp + 4, (void*)nal, sizeof_nal);
			if (is_intra)
				sample_kind = MP4E_SAMPLE_RANDOM_ACCESS;
			err = MP4E_put_sample(
				h->mux, h->mux_track_id, tmp, 4 + sizeof_nal, timeStamp90kHz_next, sample_kind);
			free(tmp);
		}
		break;
	}
		...
		default:
			...
			if (!h->need_pps && !h->need_idr) {
				...
				if (first_mb_in_slice)
					...
					sample_kind = MP4E_SAMPLE_RANDOM_ACCESS;
				err = MP4E_put_sample(
					h->mux, h->mux_track_id, nal2, sizeof_nal, timeStamp90kHz_next, sample_kind);
			}
			break;
		}
		free(nal1);
...
*/
static uint32_t
read_payload(MP4D_demux_t* mp4, unsigned nb, boxsize_t* payload_bytes, int* eof_flag) {
	if (*payload_bytes < nb) {
		*eof_flag = 1;
...
#define MALLOC(t, p, size)                                                                         \
	{                                                                                              \
		free(p);                                                                                   \
		p = (t)minimp4_bounded_malloc(size);                                                       \
		if (!(p)) {                                                                                \
			ERROR("out of memory");                                                                \
		}                                                                                          \
	}
...
int MP4D_open(MP4D_demux_t* mp4,
	...
	do {
		...
		static const struct {
			...
			{BOX_meta,
			 0,
			 0}, // Android can produce meta box without 'FullBox' field, comment this line to simulate the bug
#endif
		...
		static const struct {
			...
			{BOX_esds,
			 BOX_OD}, // TODO: BOX_esds can be used for both audio and video, but this code supports audio only!
			{OD_ESD, BOX_OD},
			{OD_DCD, BOX_OD},
			{OD_DSI, BOX_OD},
			{BOX_trak, BOX_ATOM},
		...
		broken_android_meta_hack:
			...
			if (box_bytes == 0 || // standard indication of 'till eof' size
				box_bytes ==
					(boxsize_t)0xFFFFFFFFU // some files uses non-standard 'till eof' signaling
			) {
			...
			for (i = 0; i < NELEM(g_fullbox); i++) {
				if (box_name == g_fullbox[i].name) {
					...
					if (box_name == BOX_meta) {
						if (FullAtomVersionAndFlags >= 8 &&
							FullAtomVersionAndFlags < payload_bytes) {
...
#if MP4D_TRACE_SUPPORTED
		box_path[depth] = (box_name >> 24) | (box_name << 24) | ((box_name >> 8) & 0x0000FF00) |
						  ((box_name << 8) & 0x00FF0000);
		...
		case BOX_stsc: //ISO/IEC 14496-12 Page 38. Section 8.18 - Sample To Chunk Box.
			...
			MALLOC(MP4D_sample_to_chunk_t*,
				   tr->sample_to_chunk,
				   tr->sample_to_chunk_count * sizeof(tr->sample_to_chunk[0]));
		...
		case BOX_stts: {
			...
			for (i = 0; i < count; i++) {
				...
				if (new_k > ts_capacity) {
					...
					tr->timestamp =
						(unsigned int*)realloc(tr->timestamp, ts_capacity * sizeof(unsigned));
					tr->duration =
						(unsigned int*)realloc(tr->duration, ts_capacity * sizeof(unsigned));
		...
		case BOX_co64:
			...
			MALLOC(MP4D_file_offset_t*,
				   tr->chunk_offset,
				   tr->chunk_count * sizeof(MP4D_file_offset_t));
		...
		case OD_DSI: //ISO/IEC 14496-1 Page 28. Section 8.6.5 - DecoderConfigDescriptor.
			...
			if (!tr->dsi && payload_bytes) {
				...
				for (i = 0; i < payload_bytes; i++) {
					tr->dsi[i] =
						minimp4_read(mp4, 1, &eof_flag); // These bytes available due to check above
				}
				tr->dsi_bytes = i;
				payload_bytes -= i;
				break;
...
static const void* MP4D_read_spspps(
	const MP4D_demux_t* mp4, unsigned int ntrack, int pps_flag, int nsps, int* sps_bytes) {
...
void MP4D_printf_info(const MP4D_demux_t* mp4) {
	...
	printf("\nMP4 FILE: %d tracks found. Movie time %.2f sec\n",
		   mp4->track_count,
		   (4294967296.0 * mp4->duration_hi + mp4->duration_lo) / mp4->timescale);
#define STR_TAG(name)                                                                              \
	if (mp4->tag.name)                                                                             \
	printf("%10s = %s\n", #name, mp4->tag.name)
	STR_TAG(title);
