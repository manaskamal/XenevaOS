...
void cmap_to_fb(uint8_t *out, uint8_t *in, int in_pixels)
{
    ...
    for (i = 0; i < in_pixels; i++)
    {
        ...
        {
            // Assuming RGBA8888. The alpha channel must be opaque;
            // leaving it 0 makes every pixel fully transparent and the
            // compositor shows a blank window.
            pix = (c.r << s_Fb.red.offset) |
                  (c.g << s_Fb.green.offset) |
                  (c.b << s_Fb.blue.offset);
            if (s_Fb.transp.length)
                pix |= (((1u << s_Fb.transp.length) - 1u) << s_Fb.transp.offset);
...
{
    _KePrint("[doom] MARK I_InitGraphics\n");
    int i, gfxmodeparm;
    char *mode;
