...
void *malloc(unsigned int size)
{
	...
	boundary_tag *test = (boundary_tag*)((size_t)ptr - sizeof(boundary_tag));
...
void free(void *ptr)
{
	...
	tag = (struct boundary_tag*)((size_t)ptr - sizeof(boundary_tag));
...
void*   realloc(void *p, unsigned int size)
{
	...
	tag = (boundary_tag*)((size_t)p - sizeof(boundary_tag));
	real_size = tag->size;
...
int liballoc_free(void* ptr, int pages) {
	_KeProcessHeapUnmap(ptr, static_cast<size_t>(pages) * 4096);
	return 0;
}
